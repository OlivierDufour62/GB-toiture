<footer class="col-12 container-fluid bg264d7efoot">
        <div class="col-12 mx-auto d-md-flex justify-content-around w-100 text-white">
            <div class="col-12 col-md-3 mt-2 child">
                <div class="d-flex justify-content-between">
                    <h4 class="mt-2">GB Toiture</h4>
                    <div class="col-2 d-lg-none mobilefoot mt-2">
                        <div class="hamb1 mb-2"></div>
                        <div class="hamb1 mb-2"></div>
                        <div class="hamb1"></div>
                    </div>
                </div>
                <div class="footmobile">
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Error sapiente reprehenderit harum omnis
                        cumque odit doloremque neque, consectetur incidunt, quis magnam deserunt exercitationem quos
                        nemo tempore, explicabo distinctio quam ex?</p>
                </div>
            </div>
            <div class="col-12 col-md-3 mt-2 child">
                <div class="d-flex justify-content-between">
                    <h4 class="mt-2">Nos prestations</h4>
                    <div class="col-2 d-lg-none mobilefoot mt-2">
                        <div class="hamb1 mb-2"></div>
                        <div class="hamb1 mb-2"></div>
                        <div class="hamb1"></div>
                    </div>
                </div>
                <div  class="footmobile">
                    <ul class="list-unstyled">
                        <a href="">
                            <li class="text-white">Couverture</li>
                        </a>
                        <a href="">
                            <li class="text-white">Entretien</li>
                        </a>
                        <a href="">
                            <li class="text-white">Chéneau</li>
                        </a>
                        <a href="">
                            <li class="text-white">Isolation</li>
                        </a>
                        <a href="">
                            <li class="text-white">Zinguerie</li>
                        </a>
                        <a href="">
                            <li class="text-white">Gouttière</li>
                        </a>
                        <a href="">
                            <li class="text-white">Cheminée</li>
                        </a>
                    </ul>
                </div>
                
            </div>
            <div class="col-12 col-md-3 mt-2 child">
                <div class="d-flex justify-content-between">
                    <h4 class="mt-2 mb-2">Contact</h4>
                    <div class="col-2 d-lg-none mobilefoot mt-2 mb-2">
                        <div class="hamb1 mb-2"></div>
                        <div class="hamb1 mb-2"></div>
                        <div class="hamb1"></div>
                    </div>
                </div>
                <div class="footmobile">
                    <p class="p-0 m-0"><i class="ri-mail-line m-lg-2 mr-2"></i>1A rue du bray</p>
                    <p class="p-0 m-0"><i class="ri-mail-line m-lg-2 mr-2"></i>62330 Guarbecque</p>
                    <p class="p-0 m-0"><i class="ri-phone-line m-lg-2 mr-2"></i>0660371281</p>
                    <p class="p-0 m-0"><i class="ri-at-line m-lg-2 mr-2"></i>guillaume61@live.fr</p>
                </div>
            </div>
        </div>
    </footer>